...
from student_app.models import Attendance, AttendanceReport, FeedBackStudent, Group, LeaveReportStudent, NotificationStudent, Parents, Staffs, Students, Subjects


def parent_home(request):
    student_obj = Students.objects.get(parent=request.user.id)
    attendance_total = AttendanceReport.objects.filter(student_id=student_obj).count()
    attendance_present_total = AttendanceReport.objects.filter(student_id=student_obj,status=True).count()
    if attendance_present_total > 0:
        attendance_present = (attendance_present_total * 100) / attendance_total
    else:
        attendance_present = 0
    attendance_absent_total = AttendanceReport.objects.filter(student_id=student_obj,status=False).count()
    if attendance_absent_total > 0:
        attendance_absent = (attendance_absent_total * 100) / attendance_total
    else:
        attendance_absent = 0
    groups = Group.objects.filter(id=student_obj.group_id.id).count()
    context = {
...
def parent_view_attendance(request):
    ...
    groups = Group.objects.filter(id=student.group_id.id)
...
def student_apply_leave(request):
    ...
    leave_data = LeaveReportStudent.objects.filter(parent_id=parent_obj).order_by("-id")
    context = {
...
def student_apply_leave_save(request):
    if request.method != 'POST':
        ...
        try:
            ...
            leave_report.save()
            messages.success(request, "Ta'tilga muvaffaqiyatli ariza topshirildi")
            return HttpResponseRedirect(reverse('student_apply_leave'))
        except:
...
def student_feedback(request):
    parent_obj=Parents.objects.get(admin=request.user.id)
    feedback_data=FeedBackStudent.objects.filter(parent_id=parent_obj).order_by("-id")
    context = {
...
def student_feedback_save(request):
    if request.method != 'POST':
        ...
        try:
            feedback=FeedBackStudent(parent_id=parent_obj,feedback=feedback_msg,feedback_reply="")
            feedback.save()
